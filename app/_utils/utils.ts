...
export function persianToWesternNumerals(persianNum: string) {
  ...
  return persianNum.replace(
    /[۰-۹]/g,
    (char) => arabicDigits[persianDigits.indexOf(char)],
  );
}
...
export const provinceTitleHandler = (
  status: IStatusFetch,
  item: string,
  ostan: { id: number; name: string }[],
) => {
  if (status === "succeeded") {

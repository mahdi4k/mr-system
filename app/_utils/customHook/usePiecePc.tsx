...
const ComponentCPU = dynamic(() => import("@/_components/pieces/Cpu"), {
  ssr: false,
}) as FC<IcpuProps>;
const ComponentMotherboard = dynamic(
  () => import("@/_components/pieces/Motherboard"),
  { ssr: false },
) as FC<ImotherboardProps>;
const ComponentGraphicCard = dynamic(
  () => import("@/_components/pieces/GraphicCard"),
  { ssr: false },
) as FC<IgraphicProps>;
const ComponentPower = dynamic(() => import("@/_components/pieces/Power"), {
  ssr: false,
}) as FC<IPowerProps>;

const UsePiecePc = (props: Iprop) => {
  const [activeCpu, setActiveCpu] = useState<Partial<CPU> | undefined>();
  const [activeMotherboard, setActiveMotherboard] = useState<
    Partial<Motherboard> | undefined
  >();
  const [activeGraphic, setActiveGraphic] = useState<
    Partial<Graphic> | undefined
  >();
    ...
    case "motherboard":
      return (
        <ComponentMotherboard
          setActiveMotherboard={setActiveMotherboard}
          activeMotherboard={activeMotherboard}
        />
      );
    case "graphic":
      return (
        <ComponentGraphicCard
          setActiveGraphic={setActiveGraphic}

    ...
    const addItemToTitle = (
      ...
      if (isSuccess && searchParams.has(paramKey) && data) {
        ...
        const itemPrice = data.price
          ? ` ||  ${Intl.NumberFormat("fa", {}).format(Number(data.price))} تومان`
          : "";
          ...
          <Stack mt={"sm"} gap={"sm"}>
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {motherboardData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(motherboardData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text truncate="start" fw={"bold"} fz={"xs"}>
                    {removePersianWords(motherboardData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (
            ...
            {isSuccessCpu && cpuParam ? (
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {cpuData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(cpuData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text truncate="start" fw={"bold"} fz={"xs"}>
                    {removePersianWords(cpuData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {graphicData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(graphicData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text truncate="start" fw={"bold"} fz={"xs"}>
                    {removePersianWords(graphicData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {powerData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(powerData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text truncate="start" fz={"xs"} fw={"bold"}>
                    {removePersianWords(powerData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (
            ...
            {isSuccessFan && fanParam ? (
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {fanData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(fanData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text
                    ...
                    fw={"bold"}
                  >
                    {removePersianWords(fanData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (
            ...
            {isSuccessSsd && ssdParam ? (
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {ssdData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(ssdData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text truncate="start" fz={"xs"} fw={"bold"}>
                    {removePersianWords(ssdData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (
            ...
            {isSuccessCase && caseParam ? (
              ...
              >
                <Text fw={"bold"} fz={"xs"}>
                  {caseData?.data.price
                    ? `${Intl.NumberFormat("fa", {}).format(Number(caseData.data.price))} تومان`
                    : ""}
                </Text>
                <Box w={215}>
                  <Text truncate="start" fz={"xs"} fw={"bold"}>
                    {removePersianWords(caseData?.data.name as string)}
                  </Text>
                </Box>
              </Flex>
            ) : (

...
const ChoosePartItem: FC<Props> = ({ svg, title, type, itemData, partEmpty }) => {

    const router = useRouter();
    const searchParams = useSearchParams();
    const [currentItemData, setCurrentItemData] = useState<Partial<CPU> | undefined>(itemData);
        ...
        <>
            ...
            {currentItemData ? (
                <>
                    ...
                    <Box pos={'relative'} bg={'var(--mantine-color-body)'}
                        w={{ base: '175px', lg: '255px' }}
                        pt={'20px'} style={{ borderRadius: '10px' }}
                        component='div' className={`${classes.box}`} mt={'0'} ta={'center'}>

                        <ActionIcon onClick={removeSelected} className={classes.closeButton} pos={'absolute'} variant='light' color="rgba(180, 220, 240, 0.98)" style={{ position: 'absolute', left: '4px', top: '5px', cursor: 'pointer', zIndex: '2' }}>
                            <IconX size={18} />
                        </ActionIcon>
                ...
                <Tooltip label={title}>
                    ...
                    <Box ta={'center'} w={{ base: '175px', lg: '255px' }} onClick={open} bg={'var(--mantine-color-body)'} className={classes.box}
                        py={'50px'} style={{ borderRadius: '10px' }} component='div' >

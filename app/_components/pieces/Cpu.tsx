...
const Cpu: FC<IcpuProps> = ({ setActiveCpu, activeCpu }) => {
  ...
  const cpuListFromSelectedMotherboard = useSelector(
    (state: RootState) => state.motherboard.relatedCpus,
  );
  const cpuListFromSelectedGraphic = useSelector(
    (state: RootState) => state.graphic.relatedCpus,
  );
  ...
  const titleDrawer = () => {
    return ObjectIsEmpty(selectedMotherboard) ? (
      "لیست cpu"
    ) : (
        ...
        {ObjectIsEmpty(currentCpu) ? (
          ...
          >
            ...
            <Card.Section style={{ textAlign: "center" }} mt={"md"}>
              {currentCpu.image && (
                <Image
                  alt={currentCpu.name ? currentCpu.name : ""}
                  width={80}
                  height={80}
                  src={currentCpu.image}
                />
              )}
            </Card.Section>
            <Flex
            ...
            <Tabs.Panel value="info">
              <Grid my={"md"}>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fz={"sm"}>گرافیک مجتمع</Text>
                </Grid.Col>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fw={"bold"} fz={"sm"}>
                    {currentCpu.integrated_graphic}
                  </Text>
                </Grid.Col>

                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fz={"sm"}>سوکت</Text>
                </Grid.Col>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fw={"bold"} fz={"sm"}>
                    {currentCpu.cpu_socket}
                  </Text>
                </Grid.Col>
              </Grid>
            </Tabs.Panel>

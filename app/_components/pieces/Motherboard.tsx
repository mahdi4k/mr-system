...
import {
  ...
  relatedCpuList,
} from "@/_redux/features/motherboard";
...
export type ImotherboardProps = {
  setActiveMotherboard: React.Dispatch<
    React.SetStateAction<Partial<MotherboardType> | undefined>
  >;
  activeMotherboard: Partial<MotherboardType> | undefined;
};

const Motherboard: FC<ImotherboardProps> = ({
  activeMotherboard,
  setActiveMotherboard,
}) => {
  const [opened, { open, close }] = useDisclosure(false);
  const dispatch = useDispatch();
  const [motherboardSelected, setSelectedMotherboard] =
    useState<MotherboardType[]>();
  const loadingEnd = useLoading();

  const MotherboardListFromSelectedCpu = useSelector(
    (state: RootState) => state.cpu.relatedMotherboards,
  );
  const currentMotherboard = useSelector(
    (state: RootState) => state.motherboard.selectedMotherboard,
    shallowEqual,
  );
  const {
    isSuccess,
          ...
          >
            ...
            <Card.Section style={{ textAlign: "center" }} mt={"md"}>
              {currentMotherboard.image && (
                <Image
                  alt={currentMotherboard.name ? currentMotherboard.name : ""}
                  width={80}
                  height={80}
                  src={`${currentMotherboard.image}`}
                />
              )}
            </Card.Section>
            <Flex
                ...
                <Group gap={3} justify="center" align="center" mb="xs">
                  ...
                  <Text fz={"xs"} fw={"bold"}>
                    {Intl.NumberFormat("fa", {}).format(
                      Number(currentMotherboard.price),
                    )}
                  </Text>
                  <Image
            ...
            <Tabs.Panel value="info">
              <Grid my={"md"}>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fz={"sm"}>سوکت پردازنده</Text>
                </Grid.Col>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fw={"bold"} fz={"sm"}>
                    {currentMotherboard.cpu_socket}
                  </Text>
                </Grid.Col>

                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fz={"sm"}>نوع RAM</Text>
                </Grid.Col>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fw={"bold"} fz={"sm"}>
                    {currentMotherboard.ddr3
                      ? "DDR3"
                      : currentMotherboard.ddr4
                        ? "DDR4"
                        : currentMotherboard.ddr5
                          ? "DDR5"
                          : "نامشخص"}{" "}
                  </Text>
                </Grid.Col>
                ...
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fz={"sm"}>تعداد اسلات RAM</Text>
                </Grid.Col>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fw={"bold"} fz={"sm"}>
                    {currentMotherboard.total_slot_ram}
                  </Text>
                </Grid.Col>

                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fz={"sm"}> ارتباط بیسیم</Text>
                </Grid.Col>
                <Grid.Col className={classes.borderBottomDashed} span={6}>
                  <Text fw={"bold"} fz={"sm"}>
                    {" "}
                    {currentMotherboard.wifi_support ? "دارد" : "ندارد"}
                  </Text>
                </Grid.Col>
          ...
          {isLoading
            ...
            : motherboardSelected?.map((el) => (
                <Grid.Col key={el.id} span={{ base: 6, sm: 4, md: 2 }}>
                  <Card
                    onClick={() => selectedMotherboard(el.id)}

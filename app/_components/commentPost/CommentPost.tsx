...
const CommentPost = ({ postID }: { postID: string }) => {
  ...
  const handleSubmit = async () => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_WORDPRESS_API}/comments?post`,
      {
        method: "POST",
        headers: {
    ...
    if (response.status === 201) {
      notifications.show({
        color: "green",
        title:
          "پیام شما با موفقیت ارسال شد و پس از بررسی ادمین در سایت قرار خواهد گرفت",
        message: "",
        autoClose: 8000,
  ...
  return (
    ...
    >
      ...
      <Flex mt={"lg"} justify={"flex-end"}>
        <Button loading={loading} type="submit" color="blue" variant="outline">
          ارسال پیام
        </Button>
      </Flex>
    </form>

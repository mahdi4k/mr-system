...
const ConfirmDeletePopover = ({ productId, onDelete }: { productId: number; onDelete: (id: number) => void }) => {
  const [opened, { open, close }] = useDisclosure(false);

  return (
    <Popover opened={opened} onClose={close} width={200} position="bottom" withArrow shadow="md">
      <Popover.Target>
      ...
      <Popover.Dropdown>
        ...
        <Flex justify={'flex-end'} mt={'sm'}>
          <Button ml={'sm'} color="red" variant="light" size="xs" onClick={() => onDelete(productId)}>
            بله
          </Button>

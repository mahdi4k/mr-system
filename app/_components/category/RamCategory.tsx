...
import { useGetRamsQuery } from '@/_redux/services/ramApi'
...
const RamCategory = () => {
    ...
    const { isSuccess, data = [], error, isLoading } = useGetRamsQuery({ search: searchSubmit })
    ...
    return (
        <>
            ...
            <Grid pb='md' mb={'xl'} >
                {data && data.map((data) => (
                    <CategoryLayout key={data.id} type={'rams'} data={data} children={
                        (

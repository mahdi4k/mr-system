...
export async function PATCH(req: Request) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ status: false, error: errorData.message }, { status: response.status });
        }

        const data = await response.json();
        return NextResponse.json({ status: true, message: data.message, user: data.user });

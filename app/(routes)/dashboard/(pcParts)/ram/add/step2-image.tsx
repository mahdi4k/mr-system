...
import { useAddRamImageMutation } from '@/_redux/services/ramApi';
...
const Step2Image = (props: Iprops) => {
    ...
    const [addRamImage, response] = useAddRamImageMutation();

    const currentRam = useSelector((state: RootState) => state.ram.currentRam)
        ...
        if (logo) {
            addRamImage({
                ...
                .then((val) => {
                    notifications.show({
                        color: 'green',
                        title: 'تصویر ram با موفقیت ثبت شد',
                        message: '',
                        classNames: notifClasses
                    })
                })
        }
    }, [addRamImage, currentRam.id, logo])
    return (
        <>
            <Text mt={"lg"} fz={"lg"} fw={"bold"} mr={"lg"}>افزودن تصویر برای ram {currentRam.name}  </Text>
           ...
           <Flex justify={'flex-end'} mt={'xl'}>
               <Button onClick={backToLevelOne}>افزودن رم جدید</Button>
           </Flex>
        </>
    );
};

...
import { useGetRamsQuery, useRemoveRamMutation } from '@/_redux/services/ramApi';


const ClientRam = () => {
    const [loadingSession, setLoadingSession] = useState(false)
    const session = useSession()
    const { isSuccess, data = [], error } = useGetRamsQuery({}, { skip: !loadingSession })
    const [opened, { open, close }] = useDisclosure(false);
    const [selectedRamID, setSelectedRamID] = useState<number>()
    const [removeRam, response] = useRemoveRamMutation();
        ...
        <>
            <Stack>
                <Flex justify={'space-between'} align={'center'}>
                    <Text mt={"sm"} mb={"md"} fw={"bold"} fz={"lg"}>لیست ram</Text>
                    <Link href="/dashboard/ram/add">

...
import { useGetCaseQuery, useUpdateCaseMutation } from '@/_redux/services/caseApi';
...
const Step1Form = (props: StepProps) => {
    ...
    const { data: caseData, isSuccess: isSuccessCase } = useGetCaseQuery({ id: router.id as string })
    useEffect(() => {
        if (isSuccessCase) {
    ...
    return (
        <Stack>
            <Text my={"lg"} fz={"lg"} fw={'bolder'}>ویرایش case</Text>
